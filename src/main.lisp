...
(defparameter *tessdata-directory* nil)
...
(defmacro with-base-api (api-name &body body &aux (handle (gensym)))
  "This macro creates a TessBaseAPI using TessBaseAPICreate and binds it to api-name.
   It wraps body with an UNWIND-PROTECT form to ensure that TessBaseAPIEnd and TessBaseAPIDelete
...
(defun init-tess-api (api lang)
  "Calls TessBaseAPIInit3 on api and sets lang, which should be a string naming a .traineddata
  file in *tessdata-directory*. Attempting to set a language that is not available will raise an error."
...
(defun process-pages (api truename)
  ...
  TessBaseAPI, and then uses Leptonica to load the image found at truename (which must be a string
  containing the complete path to the file). It then runs OCR on the image. On SBCL,
  the division-by-zero float trap is masked to prevent it from causing a non-recoverable error.
  The details of your Leptonica installation will determine exactly what image formats this function
  can process successfully.
  Returns T on success, otherwise signals error."
  (mask-sigfpe
    (let ((return-code (tessbaseapiprocesspages api truename "" 0 (cffi:null-pointer))))
      (case return-code
        (1 t)
...
(defun get-utf8-text (api)
  "This function calls TessBaseAPIGetUTF8Text on api, which should be an initialized
   TessBaseAPI with its image set. If called before another function such as PROCESS-PAGES
   has carried out OCR on the image, it will be carried out. On SBCL, the division-by-zero
   float trap is masked to prevent it from causing a non-recoverable error.
